...
client.on('messageCreate', async message => {
    ...
    if (message.embeds[0]?.description?.startsWith('More information about a profession with')) {
        ...
        const recentUserMessage = await message.channel?.getMessages({ before: message.id, limit: 1, filter: m => (m.author.id === id || m.content.includes(id)) && !!m.content.match(/^rpg pr/i) });
